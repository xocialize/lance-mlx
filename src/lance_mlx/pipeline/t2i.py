...
class TextToImagePipeline:
    ...
    def from_pretrained(
        ...
        Macs). `und_mode` is a deprecated alias (keep→parallel, deferred/free→relay)."""
    ...
    def generate(
        ...
        *,
        ...
        Args:
            ...
            free_und: whether to free the UND tower (~5.5 GB) after prefill. None
                (default) auto-detects from the device memory budget — frees on a
                ~16 GB Mac, keeps it resident on a 24 GB+ machine. True/False
                override. Freeing is destructive (the pipeline becomes single-shot)
                but does not change the output: the GEN-only loop runs identically
                whether or not UND is resident.
        ...
        if tile_vae:
            ...
            # free_und holds the denoise loop flat at 7.4 GB. 256px tiles (16
