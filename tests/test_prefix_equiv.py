...
by BOTH t2i and t2v — the mechanism lives in the shared ``LanceModel`` methods
(``prefill_prefix`` / ``gen_loop_forward`` / ``free_und_tower``), which t2v
reuses unchanged, so one test covers both.
...
def test_gen_loop_with_und_resident_matches_baseline(P: int, n_lat: int):
    """parallel (UND-resident) path: the GEN-only loop is gated on the prefix
    caches, NOT on the UND tower being freed, so it must reproduce the baseline
    velocity with the UND weights STILL RESIDENT (no free_und_tower call). This is
    the guarantee auto relies on — keeping the tower never changes output."""
    mx.random.seed(0)
    cfg = _tiny_config()
    model = LanceModel(cfg, num_latent_positions=64)
    ...
    # NO free_und_tower() — UND stays resident (the parallel case).
    h_lat = model.gen_loop_forward(latent, position_ids=pos[:, :, P:], caches=caches)
    v_opt = model.llm2vae(h_lat)
...
def test_resolve_memory_mode_bands(monkeypatch):
    """resolve_memory_mode: explicit modes verbatim and the binary auto bands
    (parallel above the budget pivot, relay below) + unresolved fail-safe."""
    ...
    # 2. Auto-detect is BINARY off the device budget — parallel or relay.
