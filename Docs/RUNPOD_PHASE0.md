...
- **`UserWarning: A NumPy version >=1.22.4 and <2.3.0 is required for this version of SciPy (detected version 2.4.4)` printed at every script startup.** Benign. scipy 1.13 was packaged with a conservative numpy upper bound of <2.3; the torch 2.8 re-upgrade transitively pulls numpy to 2.4.x. Empirically all 5 task scripts run to clean completion through this warning. scipy isn't on Lance's critical path (it comes in via librosa / scikit-image). Don't try to "fix" it by force-downgrading numpy — that risks cascading conflicts with torch 2.8's own numpy expectations.
